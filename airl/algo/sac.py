...
class SAC(Algorithm):
    ...
    def __init__(self, state_shape, action_shape, device, seed, gamma=0.99,
                 ...
                 units_critic=(256, 256), start_steps=10000, tau=5e-3, alpha_init=0.2):
    ...
    def update_sac(self, states, actions, rewards, dones, next_states,
                   writer):
        self.learning_steps += 1
        self.update_critic(
            states, actions, rewards, dones, next_states, writer)
    ...
    def update_critic(self, states, actions, rewards, dones, next_states,
        ...
        if self.learning_steps % 1000 == 0:
            writer.add_scalar('loss/critic1', loss_critic1.item(), self.learning_steps_disc*self.rollout_length)
            writer.add_scalar('loss/critic2', loss_critic2.item(), self.learning_steps_disc*self.rollout_length)
    ...
    def update_actor(self, states, writer):
        ...
        if self.learning_steps % 1000 == 0:
            writer.add_scalar(
                'loss/actor', loss_actor.item(), self.learning_steps_disc*self.rollout_length)
            writer.add_scalar(
                'loss/alpha', loss_alpha.item(), self.learning_steps_disc*self.rollout_length)
            writer.add_scalar(
